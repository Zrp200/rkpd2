...
import com.zrp200.rkpd2.actors.buffs.GravityChaosTracker;
...
import com.zrp200.rkpd2.actors.buffs.SuperNovaTracker;
...
public class CursedWand {
	...
	public static abstract class CursedEffect {
		...
		public void FX(final Item origin, final Char user, final Ballistica bolt, final Callback callback){
			if (user == null) {
				if (!Objects.equals(bolt.collisionPos, bolt.sourcePos)) {
					new MagicMissile().reset(
							MagicMissile.RAINBOW,
							DungeonTilemap.tileCenterToWorld(bolt.sourcePos),
							Actor.findChar(bolt.collisionPos).sprite.center(),
							callback
					);
				} else {
					callback.call();
	...
	// fixme this is just until I figure out how to assume that the user doesn't exist for many of the new curse effects
	private static abstract class UserAffectingCurseEffect extends CursedEffect {
		public boolean valid(Item origin, Char user, Ballistica bolt, boolean positiveOnly){
			return user != null;
		}
	}
	...
	public static class RandomTeleport extends CursedEffect {
		// fixme double check validity after merge
		@Override
		public boolean valid(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			Char target = Actor.findChar(bolt.collisionPos);
            return (!positiveOnly && user != null) || target != null && !Char.hasProp(target, Char.Property.IMMOVABLE);
        }
	...
	public static class Bubbles extends UserAffectingCurseEffect {
	...
	public static class HealthTransfer extends CursedEffect {
		...
		@Override
		public boolean valid(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			return user != null && Actor.findChar( bolt.collisionPos ) != null;
		}
	...
	// fixme vfx
	public static class LightningBolt extends UserAffectingCurseEffect {


		@Override
		public boolean valid(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			// fixme
			return super.valid(origin, user, bolt, positiveOnly) && user != null;
		}
	...
	public static class InterFloorTeleport extends UserAffectingCurseEffect {
	...
	public static class SinkHole extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			PathFinder.buildDistanceMap( bolt.sourcePos, passable, 5 );
